...
// Desktop client does not have data-testid attributes, but the classes are not obfuscated. We map testid values to query selectors.
const testIdMappingForDesktopClient = {
  "control-button-repeat": ".player-controls .main-repeatButton-button",
  "control-button-shuffle": ".player-controls button:nth-of-type(1)",
  "context-item-info-subtitles": ".main-trackInfo-artists",
  "context-item-link": ".main-trackInfo-name a",
  "playback-position": ".playback-bar :first-child",
  "playback-duration": ".playback-bar .main-playbackBarRemainingTime-container",
  "now-playing-bar": ".main-nowPlayingBar-container",
  "now-playing-widget": ".main-nowPlayingWidget-nowPlaying",
}

function getElementByTestID(testid) {
  const byTestid = document.querySelector(`[data-testid="${testid}"]`)
  if (byTestid) return byTestid
  const byQuerySelector = document.querySelector(
    testIdMappingForDesktopClient[testid]
  )
  if (byQuerySelector) return byQuerySelector
  console.error(
    `[spotify-browser-metadata-script] testid ${testid} not found, and ${testIdMappingForDesktopClient[testid]} not found either`
  )
  return null
}
...
function durationStringToSeconds(durationString) {
  ...
  return (
    parseInt(seconds) +
    60 * parseInt(minutes) +
    3600 * (hours === undefined ? 0 : parseInt(hours))
  )
}
...
function updateState(mutations) {
  ...
  state.repeat =
    getElementByTestID("control-button-repeat")?.getAttribute(
      "aria-checked"
    ) !== "false"
  state.loop =
    getElementByTestID("control-button-repeat")?.getAttribute(
  ...
  state.liked =
    getElementByTestID("now-playing-widget")
      .querySelector('button[aria-label^="Add to"]')
      ?.getAttribute("aria-checked") === "true"
  const shuffleLabel = getElementByTestID(
    "control-button-shuffle"
  )?.getAttribute("aria-label")
  state.shuffle =
    shuffleLabel.startsWith("Enable Smart") ||
    shuffleLabel.startsWith("Disable")
  ...
  state.playing =
    document
      .querySelector(".player-controls__buttons")
      .querySelectorAll('[aria-label="Pause"]').length > 0
  state.elapsed = durationStringToSeconds(
    getElementByTestID("playback-position")?.textContent
  )
  state.duration = durationStringToSeconds(
    getElementByTestID("playback-duration")?.textContent
  )
  state.current_lyric = ""
  state.upcoming_lyric = ""
  const transformLyric = (lyric) =>
    lyric.replace(/x(\d+)/, ($0, $1) => `×${$1}`)
  const lyricsContainer = document.querySelector("main > [style*=lyrics]")
  if (
    lyricsContainer &&
    !lyricsContainer.querySelector("p")?.innerText?.includes("sync")
  ) {
    const activeLyricColor = getComputedStyle(lyricsContainer).getPropertyValue(
      "--lyrics-color-active"
    )
    for (const lyricElement of document.querySelectorAll(
      "main > [style*=lyrics] div[dir=auto]"
    )) {
      if (
        getComputedStyle(lyricElement).getPropertyValue("color") ===
          activeLyricColor &&
        getComputedStyle(lyricElement).getPropertyValue("opacity") === "1"
      ) {
        state.current_lyric = transformLyric(lyricElement?.textContent || "")
        state.upcoming_lyric = transformLyric(
          lyricElement?.nextSibling.textContent || ""
        )
        break
      }
    }
  ...
  request.open(
    "GET",
    `${PROTOCOL}://${HOST}:${PORT}/` + stateToQuerystring(state)
  )
  request.send()
...
console.log(
  '[spotify-browser-metadata-script] waiting for [data-testid="now-playing-widget"]...'
)
document.arrive('[data-testid="now-playing-widget"]', startObserving)

console.log(
  `[spotify-browser-metadata-script] waiting for ${testIdMappingForDesktopClient["now-playing-widget"]}...`
)
document.arrive(
  testIdMappingForDesktopClient["now-playing-widget"],
  startObserving
)
